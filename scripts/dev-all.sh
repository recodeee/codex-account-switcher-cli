...
RUNTIME_LAYER="$(printf "%s" "$RUNTIME_LAYER_RAW" | tr "[:upper:]" "[:lower:]")"
case "$RUNTIME_LAYER" in
  python|rust) ;;
  *)
    echo "[dev] Unsupported RUNTIME_LAYER='${RUNTIME_LAYER_RAW}'. Use 'python' or 'rust'." >&2
    exit 1
    ;;
esac
